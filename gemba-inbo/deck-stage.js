...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Click-to-reveal fragments live in the light DOM, so their styles
      // must too (shadow stylesheet can't reach slotted slide content).
      // Injected once per document, shared by all deck-stage instances.
      // Print forces every fragment visible — the handout is the end state.
      if (!document.getElementById('deck-stage-reveal-css')) {
        const revealCss = document.createElement('style');
        revealCss.id = 'deck-stage-reveal-css';
        revealCss.textContent = `
deck-stage [data-reveal] { opacity: 0; visibility: hidden; transition: opacity .45s ease, visibility 0s linear .45s; }
deck-stage [data-reveal][data-reveal-shown] { opacity: 1; visibility: visible; transition: opacity .45s ease, visibility 0s; }
@media (prefers-reduced-motion: reduce) { deck-stage [data-reveal] { transition: none; } }
@media print { deck-stage [data-reveal] { opacity: 1 !important; visibility: visible !important; transition: none !important; } }
`;
        document.head.appendChild(revealCss);
      }
      // Fullscreen presentation: hide the rail + nav overlay whenever the
      // page is fullscreen. fullscreenchange covers requestFullscreen (the
      // F key) and F11; macOS's green-button/Cmd-Ctrl-F fullscreen never
      // fires the Fullscreen API, so _syncFsPresenting also runs a
      // viewport==screen heuristic from the resize handler.
      this._onFullscreenChange = () => this._syncFsPresenting();
      document.addEventListener('fullscreenchange', this._onFullscreenChange);
      document.addEventListener('webkitfullscreenchange', this._onFullscreenChange);
      this._syncFsPresenting();
    ...
    /** True fullscreen detection for standalone decks (no host SPA).
     *  Fullscreen API first; falls back to viewport==screen, which is the
     *  only signal Chrome on macOS gives for green-button/Cmd-Ctrl-F
     *  fullscreen. Mirrors the __omelette_presenting side effects. */
    _syncFsPresenting() {
      const fsEl = document.fullscreenElement || document.webkitFullscreenElement;
      const fsHeuristic = Math.abs(window.innerWidth - screen.width) <= 1
        && Math.abs(window.innerHeight - screen.height) <= 1;
      const next = !!fsEl || fsHeuristic;
      if (next === !!this._fsPresenting) return;
      this._fsPresenting = next;
      if (next && this._overlay) {
    ...
    /** Keyboard twin of the TweaksPanel __deck_rail_visible message —
     *  same persisted preference, same armed transition. */
    _toggleRail() {
    ...
    _flashOverlay() {
      ...
      // flash) so the audience sees clean slides. _fsPresenting is the
      // standalone-file equivalent (browser fullscreen, no host).
      if (!this._overlay || this._presenting || this._fsPresenting) return;
    ...
    _railWidth() {
      ...
      if (!this._railEnabled || !this._railVisible || this.hasAttribute('no-rail')
          || this.hasAttribute('noscale') || this._presenting || this._fsPresenting
          || this._previewMode || NARROW_MQ.matches) return 0;
    ...
    _onResize() {
      // macOS green-button fullscreen only announces itself as a resize.
      this._syncFsPresenting();
      this._fit();
    ...
    _syncRailHidden() {
      ...
      // :host([data-rail-anim]).
      const hard = !this._railEnabled || this._presenting || this._fsPresenting || this._previewMode;
    ...
    // ── Click-to-reveal fragments ─────────────────────────────────────────
    //
    // Elements marked data-reveal start hidden (style injected in
    // connectedCallback); forward nav shows them one step at a time before
    // leaving the slide, backward nav re-hides the most recent step before
    // going back. data-reveal="N" orders steps; equal N reveal together;
    // bare data-reveal is step 0. Reveals never call _go/_applyIndex, so no
    // slidechange fires and deck-morph is untouched. State persists across
    // revisits; direct jumps (number keys, Home/End, rail clicks) leave it
    // as-is.

    _revealStep(el) {
      const n = parseFloat(el.getAttribute('data-reveal'));
      return isNaN(n) ? 0 : n;
    }
    ...
    _revealNext() {
      ...
      const pending = Array.from(slide.querySelectorAll('[data-reveal]:not([data-reveal-shown])'));
      if (!pending.length) return false;
      const step = Math.min(...pending.map((el) => this._revealStep(el)));
      pending.filter((el) => this._revealStep(el) === step)
    ...
    _revealPrev() {
      const slide = this._slides[this._index];
      if (!slide) return false;
      const shown = Array.from(slide.querySelectorAll('[data-reveal][data-reveal-shown]'));
      if (!shown.length) return false;
      const step = Math.max(...shown.map((el) => this._revealStep(el)));
      shown.filter((el) => this._revealStep(el) === step)
        .forEach((el) => el.removeAttribute('data-reveal-shown'));
      return true;
    }
     ...
     *  nothing further in that direction. Pending reveal fragments on the
     *  current slide consume the step before any slide change happens. */
    _advance(dir, reason) {
      if (!this._slides.length) return;
      if (dir === 1 && this._revealNext()) return;
      if (dir === -1 && this._revealPrev()) return;
      let i = this._index + dir;
      while (i >= 0 && i < this._slides.length && this._slides[i].hasAttribute('data-deck-skip')) {
        i += dir;
      }
      if (i < 0 || i >= this._slides.length) { this._flashOverlay(); return; }
      // Arriving backward lands on the slide fully revealed — that is the
      // state it was in when the presenter left it going forward.
      if (dir === -1) {
        this._slides[i].querySelectorAll('[data-reveal]')
          .forEach((el) => el.setAttribute('data-reveal-shown', ''));
      }
      this._go(i, reason);
    }
